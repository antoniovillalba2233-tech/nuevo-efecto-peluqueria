...
import { PRODUCCION } from '@/data/contenido';
...
/**
 * NUESTRA PRODUCCIÓN
 * Mosaico con lightbox para mostrar cortes, peinados y otros trabajos.
 * ⚠️ Para agregar o cambiar fotos: src/data/contenido.ts → PRODUCCION
 */
const Produccion: React.FC = () => {
  ...
  const anterior = useCallback(
    () => setIndice((i) => (i === null ? null : (i - 1 + PRODUCCION.length) % PRODUCCION.length)),
    [],
  );
  const siguiente = useCallback(
    () => setIndice((i) => (i === null ? null : (i + 1) % PRODUCCION.length)),
    [],
  );
  ...
  const foto = indice !== null ? PRODUCCION[indice] : null;

  return (
    <section id="produccion" className="scroll-mt-20 bg-carbon py-20 text-crema sm:py-28">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <Reveal className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-2xl">
            <p className="kicker text-oro">Nuestros cortes</p>
            <h2 className="titulo-display mt-3 text-4xl leading-[1.05] sm:text-5xl lg:text-6xl">
              Nuestra <span className="italic text-oro">producción</span>
            </h2>
          </div>
          <p className="max-w-sm text-sm leading-relaxed text-crema/60">
            Cortes, peinados y trabajos recientes del equipo. Tocá cualquier foto para verla en
            grande.
          </p>
        </Reveal>
          ...
          {PRODUCCION.map((item, i) => (
            <Reveal key={`${item.src}-${i}`} delay={(i % 3) * 90} className="mb-5 break-inside-avoid">
              <button
                ...
                onClick={() => {
                  setIndice(i);
                  track('gallery_open', { foto: item.titulo, seccion: 'produccion' });
                }}
